...
class CifarBatchTransform:
        ...
        **kwargs,
        ...
        if train_transform:
            ...
            if strong_aug:
                ...
                lst_of_transform = [
                    transforms.RandomResizedCrop(32, scale=(0.3, 0.85)),
                    transforms.RandomHorizontalFlip(p=0.5),
                    transforms.RandomApply(
                        [transforms.ColorJitter(0.7, 0.7, 0.5, 0.4)], p=0.9
                    ),
                    transforms.RandomGrayscale(p=0.2),
                    GaussianBlur(0.75),
                    Solarization(0.5),
    ...
    def __call__(self, x):
        ...
        y = einops.rearrange(y, "(A B C) D -> D A B C", B=patch_size, C=patch_size)
        # y = torch.zeros(self.n_transform, C_aug, H_aug, W_aug)
        # for i in range(self.n_transform):
        #     y[i, :, :, :] = self.transform(x)
        return y
